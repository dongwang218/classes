...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    logits, cache_ta = temporal_affine_forward(h, W_vocab, b_vocab)
    loss, dlogits = temporal_softmax_loss(logits, captions_out, mask)
    dh, dW_vocab, db_vocab = temporal_affine_backward(dlogits, cache_ta)
    if self.cell_type == 'rnn':
  ...
  def sample(self, features, max_length=30):
    ...
    for i in xrange(max_length):
      ...
      logits, _ = temporal_affine_forward(next_h.reshape(N, 1, next_h.shape[1]), W_vocab, b_vocab)
      next_x = np.squeeze(np.argmax(logits, axis = 2))
      captions[:,i] = next_x
      x = W_embed[next_x]
      prev_h = next_h
