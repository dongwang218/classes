...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #exp = np.exp(scores)
    #scores = exp / np.sum(exp, axis = 1).reshape((num_train, 1))
    ...
    scores -= np.max(scores, axis = 1).reshape((num_train, 1))
    exp = np.exp(scores)
    normalize = exp / np.sum(exp, axis = 1, keepdims = True)
    ...
    #############################################################################
    # use method in http://cs231n.github.io/neural-networks-case-study/
    probs = normalize
    dscores = probs
    dscores[np.arange(num_train), y] -= 1
    dscores /= num_train
    dW = np.dot(hidden.T, dscores)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      for param in self.params:
        self.params[param] -= grads[param].reshape(self.params[param].shape) * learning_rate
