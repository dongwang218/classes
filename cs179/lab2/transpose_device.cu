...
void naiveTransposeKernel(const float *input, float *output, int n) {
    ...
    // input is aligned, output is not
...
void shmemTransposeKernel(const float *input, float *output, int n) {
    ...
    // still has 32 band conflict, does not seem to affect performance!
    ...
    // tranpose then store in shmem
    for (index = 0; index < 4; index++)
      data[(4*threadIdx.y+index) + 64 * threadIdx.x] = input[i + n * (j+index)];
    __syncthreads();

    // recalculate the target address
    const int i2 = threadIdx.x + 64 * blockIdx.y;
    int j2 = 4 * threadIdx.y + 64 * blockIdx.x;
    for (index = 0; index < 4; index++)
      output[i2 + n * (j2 + index)] = data[threadIdx.x + 64 * (4*threadIdx.y+index)];
}
...
void optimalTransposeKernel(const float *input, float *output, int n) {
    ...
    // tranpose then store in shmem
    for (index = 0; index < 4; index++)
      data[threadIdx.x][(4*threadIdx.y+index)] = input[i + n * (j+index)];
    __syncthreads();
    ...
    int j2 = 4 * threadIdx.y + 64 * blockIdx.x;
    for (index = 0; index < 4; index++)
      output[i2 + n * (j2 + index)] = data[4*threadIdx.y+index][threadIdx.x];
}
...
void optimalTransposeKernel(const float *input, float *output, int n) {
    ...
    // tranpose then store in shmem
    data[(4*threadIdx.y) + 64 * threadIdx.x] = input[i + n * (j)];
    ...
    const int i2 = threadIdx.x + 64 * blockIdx.y;
    int j2 = 4 * threadIdx.y + 64 * blockIdx.x;
    output[i2 + n * (j2)] = data[threadIdx.x + 64 * (4*threadIdx.y)];
    output[i2 + n * (j2 + 1)] = data[threadIdx.x + 64 * (4*threadIdx.y+1)];
    output[i2 + n * (j2 + 2)] = data[threadIdx.x + 64 * (4*threadIdx.y+2)];
    output[i2 + n * (j2 + 3)] = data[threadIdx.x + 64 * (4*threadIdx.y+3)];
    }*/
