...
template<typename T> void cudaClip(T *dev_ptr, T max_value, int n_vals)
{
  thrust::device_ptr<T> d_n(dev_ptr, dev_ptr+n_vals);
  thrust::device_vector<float> temp(n_vals);
  thrust::fill(temp.begin(), temp.end(), max_value);

  thrust::transform(d_n.begin(),d_n.end(), temp.begin(), d_n.begin(), min_functor<T>());
}
...
float CrossEntropyLoss(float* pred_Y, float* true_Y, int n, int c, int h, int w)
{
    ...
    CrossEntropyKernel<<<n_blocks, BW, sizeof(float)*BW>>>(pred_Y, true_Y, d_loss, n, c, h, w);
    ...
    int n, int c, int h, int w)
{
    ...
    // Accumulate the total loss on the device by invoking a kernel
    int n_blocks = std::min(65535, (n + BW - 1) / BW);
    ...
    int n, int c, int h, int w)
{
    ...
    {
      //if (true_Y[idx] > 0) { // 0 * log(0) gives nan, dong fix
