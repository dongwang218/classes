...
/******************************************************************************/
const float MAX_DATA = 1e2;
void print_cuda(const char* name, float* ptr, int size, bool force=false) {
  bool first = true;
  float* host_data = (float*)malloc(sizeof(float) * size);
    CUDA_CALL(cudaMemcpy(host_data, ptr, sizeof(float)*size, cudaMemcpyDeviceToHost));
    for (int i = 0; i < size; ++i) {
      if (force || host_data[i] > MAX_DATA) {
        if (first) {
          printf("%s ", name);
        }
...
void Layer::init_weights_biases()
{
    ...
    {
      float *onevec;
          CUDA_CALL( cudaMalloc(&onevec, n_weights * sizeof(float)) );
          cudaMemsetType<float>(onevec, 1.0f, n_weights);

        CURAND_CALL( curandGenerateUniform(gen, weights, n_weights) );
        CUBLAS_CALL( cublasSaxpy(cublasHandle, n_weights,
            &minus_half, onevec, 1, weights, 1) );
...
void Layer::print_fwd() {
    cudnnDataType_t dtype;
    int n, c, h, w, n_stride, c_stride, h_stride, w_stride;
    CUDNN_CALL( cudnnGetTensor4dDescriptor(out_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride) );
    int size = n*c*h*w;
    print_cuda("fwd", out_batch, size);
}


void Layer::print_bwd() {
    cudnnDataType_t dtype;
    int n, c, h, w, n_stride, c_stride, h_stride, w_stride;
    CUDNN_CALL( cudnnGetTensor4dDescriptor(out_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride) );
    int size = n*c*h*w;
    char buffer [500];
    sprintf(buffer, "bwd n %d c %d h %d w %d ", n, c, h, w);
    print_cuda(buffer, grad_out_batch, size);
...
void Dense::forward_pass()
{
    ...
    // print_cuda("dense weights", weights, out_size * in_size);
    // print_cuda("dense bias", biases, out_size);

    // TODO (set 5): out_batch = weights^T * in_batch (without biases)
    // seems weight_(out x in) * in_batch is better
...
void Dense::backward_pass(float learning_rate)
{
    ...
    // grad_weights = grad_out_batch * in_batch^T
    CUBLAS_CALL( cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T,
    ...
    // grad_in_batch = W^T * grad_out_batch
    ...
    CUBLAS_CALL( cublasSaxpy(cublasHandle, out_size,
...
: Layer(prev, cublasHandle, cudnnHandle)
{
    ...
    CUDNN_CALL( cudnnGetTensor4dDescriptor(in_shape, &dtype, &n, &c, &h, &w,
        &n_stride, &c_stride, &h_stride, &w_stride) );
    std::cout << "conv2d in_shape n " << n << " c " << c << " h " << h << " w " << w << " h_stride " << h_stride << " w_stride " << w_stride << std::endl;
    ...
    CUDNN_CALL(cudnnSetFilter4dDescriptor(filter_desc,
    ...
    CUDNN_CALL( cudnnSetTensor4dDescriptor(weight_desc, CUDNN_TENSOR_NCHW,
                                           CUDNN_DATA_FLOAT, n_kernels, c, kernel_size, kernel_size) );
    ...
    std::cout << "conv2d out_shape n " << n << " c " << c << " h " << h << " w " << w << std::endl;
...
void Conv2D::backward_pass(float learning_rate)
{
    ...
    // https://docs.nvidia.com/deeplearning/sdk/cudnn-api/index.html#cudnnConvolutionBackwardFilter
    ...
    CUDNN_CALL( cudnnAddTensor(cudnnHandle,
                               &eta, weight_desc, grad_weights,
                               &one, weight_desc, weights) );
    // TODO (set 6): biases = biases + eta * grad_biases
    CUDNN_CALL( cudnnAddTensor(cudnnHandle,
                               &eta, bias_desc, grad_biases,
                               &one, bias_desc, biases) );
}
...
void SoftmaxCrossEntropy::backward_pass(float lr)
{
    ...
    //               cublasSaxpy
    CUBLAS_CALL( cublasSaxpy(cublasHandle, size, &minus_one, grad_out_batch, 1,
                             grad_in_batch, 1) );
...
float SoftmaxCrossEntropy::get_loss()
{
    ...
    int size = n*c*h*w;
    // print_cuda("softmax out_batch", out_batch, size, false);
    // print_cuda("softmax grad_out_batch", grad_out_batch, size, false);
    // printf("loss %f\n", loss);
    return loss;
}
