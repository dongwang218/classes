...
float cuda_call_blur_kernel(const unsigned int blocks,
    ...
    float* gpu_raw_data;
    gpu_errchk(cudaMalloc(&gpu_raw_data, n_frames * sizeof(float)));
    ...
    //       allocated.
    float* gpu_blur_v;
    gpu_errchk(cudaMalloc(&gpu_blur_v, blur_v_size * sizeof(float)));
    ...
    float* gpu_out_data;
    gpu_errchk(cudaMalloc(&gpu_out_data, n_frames * sizeof(float)));
