...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        next = index + 1
        delta_x = spath[next][0] - spath[index][0]
        delta_y = spath[next][1] - spath[index][1]
        rx = estimate[0] - spath[index][0]
        ry = estimate[1] - spath[index][1]
        #print(index, rx, ry, delta_x, delta_y, spath[next][0], spath[next][1], spath[next][1], spath[index][1])
        cte = (ry * delta_x - rx * delta_y) / (delta_x ** 2 + delta_y ** 2)
        if rx * delta_x + ry * delta_y > delta_x**2 + delta_y**2:
          index = next
