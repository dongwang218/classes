...
# generate a random set of distinctive landmarks,
# robot will move randomly and save its noisy movement and noisy measurements
# to visible landmarks as data, goal is to discover all the landmarks.
#
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Xi = matrix()
    Xi.zero(N+num_landmarks, 2)
    Omega.value[0][0] += 1
    Xi.value[0][0] += world_size / 2.0
    Xi.value[0][1] += world_size / 2.0
    ...
    for i in range(N-1):
      ...
      Xi.value[i][0] += - data[i][-1][0] / motion_noise
      Xi.value[i][1] += - data[i][-1][1] / motion_noise
      Xi.value[i+1][0] += data[i][-1][0] / motion_noise
      Xi.value[i+1][1] += data[i][-1][1] / motion_noise
      ...
      for measurement in data[i][0]:
        ...
        Omega.value[j][j] += 1  / measurement_noise
        Omega.value[i][j] -= 1 / measurement_noise
        Omega.value[j][i] -= 1 / measurement_noise
        Xi.value[i][0] += - x / measurement_noise
        Xi.value[i][1] += - y / measurement_noise
        Xi.value[j][0] += x / measurement_noise
        Xi.value[j][1] += y / measurement_noise

    mu2 = Omega.inverse() * Xi

    mu = matrix()
    mu.zero(2*(N+num_landmarks), 1)
    for i in range(N+num_landmarks):
      mu.value[2*i][0] = mu2.value[i][0]
      mu.value[2*i+1][0] = mu2.value[i][1]
