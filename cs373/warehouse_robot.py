...
def plan(warehouse, dropzone, todo):
    ...
    def heuristic(goal, source):
      dx = abs(goal[0] - source[0])
      dy = abs(goal[1] - source[1])
      return min(dx+dy, math.sqrt(dx**2 + dy**2))
      ...
      for row in range(num_rows):
        for col in range(num_cols):
          if wh[row][col] == box:
            ...
            goal.append(col)
            break
      closed = [[0 for col in range(num_cols)] for row in range(num_rows)]
      open = [(0 + heuristic(dropzone, goal), 0, dropzone[0], dropzone[1])]
      #print('goal', goal)
      while open:
        open = sorted(open)
        expand = open.pop(0)
        x, y = expand[2], expand[3]
        if closed[x][y] == 1: continue

        #print('expand x, y', x, y)
        closed[x][y] = 1
        if expand[2] == goal[0] and expand[3] == goal[1]:
          cost += 2*expand[1]
          #print('found %s with cost %s' % (goal, expand[1]))
          break
        for dx in [-1, 0, 1]:
          for dy in [-1, 0, 1]:
            ...
            x2 = x + dx
            y2 = y + dy
            if x2 >= 0 and x2 < num_rows and y2 >= 0 and y2 < num_cols and wh[x2][y2] in [0, box] and closed[x2][y2] == 0:
              g = expand[1] + (1 if (dx == 0) or dy == 0 else 1.5)
              h = heuristic([x2, y2], goal)
              open.append((g+h, g, x2, y2))
        #print('open', open)
      wh[goal[0]][goal[1]] = 0
    return cost
