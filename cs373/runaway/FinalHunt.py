...
def estimate_next_pos(measurement, steps=1, OTHER = None):
  ...
  if OTHER is None:
    ...
    sigma =  matrix([[1000, 0, 0, 0, 0],
                 [0, 1000, 0, 0, 0],
                 [0, 0, 1000, 0, 0],
                 [0, 0, 0, 1000, 0],
                 [0, 0, 0, 0, 1000]])# initial uncertainty: 0 for positions x and y, 1000 for the two velocities
...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER = None):
    ...
    targets_next, OTHER = estimate_next_pos(target_measurement, 5, OTHER)
    goal = targets_next[-1]
    ...
    #print('target', target_measurement, 'hunter_position', hunter_position, 'goal', goal, 'distance', distance)
...
# can only get to 0.5, for large noise, kalman filter prediction is confined in a much smaller cycle.
measurement_noise = 0.5 * target.distance #2.0*target.distance # VERY NOISY!!
