...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    x_c_s = DIGIT_DATUM_WIDTH / 4
    y_c_s = DIGIT_DATUM_HEIGHT / 4
    total = 0
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            x_bar = x / x_c_s
            ...
            if datum.getPixel(x, y) > 0:
                total += 1
                features[(-x_bar,-y_bar)] += 1
            else:
                features[(-x_bar,-y_bar)] = 0
    for x_bar in range(4):
      for y_bar in range(4):
        features[(-x_bar,-y_bar)] /= float(total)
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    # copy evaluationFunction in multiagent
    ...
    features['too_close'] = 1 if distances_bad and min(distances_bad) <= 2 else 0
    features['eat_capsule'] =  1 if newPos in currentGameState.getCapsules() else 0
    distances = [manhattanDistance(newPos, (x1, y1)) for x1, y1 in newFood.asList()]
    features['distance_food'] = min(distances) if distances else 0
