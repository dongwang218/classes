...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
          capture = self.getJailPosition()
          allPossible[capture] = 1.0
        else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos, oldProb in self.beliefs.iteritems():
          if oldProb > 0:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newPosDist.divideAll(1.0/oldProb)
            new_belief += newPosDist
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        total = self.numParticles / len(self.legalPositions)
        self.particles = self.legalPositions * total
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
          capture = self.getJailPosition()
          self.particles = [capture] * self.numParticles
        else:
          weighted = util.Counter()
          for pos in self.particles:
            trueDistance = util.manhattanDistance(pos, pacmanPosition)
            if emissionModel[trueDistance] > 0:
              weighted[pos] += emissionModel[trueDistance]
          if weighted:
            self.particles = [util.sample(weighted) for i in range(self.numParticles)]
          else:
            self.initializeUniformly(gameState)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        base_len = len(particles)
        total = int(math.ceil(self.numParticles / float(base_len)))
    ...
    def observeState(self, gameState):
        ...
        for pos in self.particles:
          ...
          for i in range(self.numGhosts):
            if noisyDistances[i] is not None:
              trueDistance = util.manhattanDistance(pos[i], pacmanPosition)
              weight *= emissionModels[i][trueDistance]
          if weight > 0:
            # a particle is killed if one elment is zero unfortunately
            weighted[pos] += weight
        if weighted:
          self.particles = [util.sample(weighted) for i in range(self.numParticles)]
        else:
        ...
        for pos in self.particles:
          ...
          with_eaten.append(pos)
        self.particles = with_eaten
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            oldState = setGhostPositions(gameState, oldParticle)

            for i in range(self.numGhosts):
              newPosDist = getPositionDistributionForGhost(oldState, i, self.ghostAgents[i])
