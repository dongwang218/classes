...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        results = [(action, self.getQValue(state, action)) for action in self.getLegalActions(state)]
        if results:
          return max([x for _, x in results])
        else:
          return 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        results = [(action, self.getQValue(state, action)) for action in self.getLegalActions(state)]
        if results:
          max_value = max([x for _, x in results])
          max_actions = [x for x, v in results if v == max_value]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        change = self.alpha * (new_q - old_q)
        feats = self.featExtractor.getFeatures(state, action)
        for f, v in feats.iteritems():
          self.weights[f] += change * v
