...
def compute_camera_matrix(real_XY, front_image, back_image):
    ...
    # (X Y Z 1 0 0 0 0)' p = x
    # (0 0 0 0 X Y Z 1)' p = y
    XY = np.tile(real_XY, (2, 1))
    num_pts = real_XY.shape[0]
    A = np.zeros((2*num_pts * 2, 8))
    b1 = front_image.reshape(front_image.shape[0] * 2)
    b2 = back_image.reshape(back_image.shape[0] * 2)
    b = np.hstack((b1, b2))
    A[::2, :2] = XY
    A[1::2, 4:6] = XY
    A[::2, 3] = 1
    A[1::2, -1] = 1
    A[(num_pts*2)::2, 2]= 150
    A[(num_pts*2+1)::2, -2]= 150
    m = np.linalg.lstsq(A, b)[0]
    return np.vstack((m.reshape((2, -1)), np.array([0,0,0, 1])))
'''
RMS_ERROR
Arguments:
...
def rms_error(camera_matrix, real_XY, front_image, back_image):
    ...
    XY = np.tile(real_XY, (2, 1))
    a = np.array([0, 1])
    a = np.tile(a, (num_pts, 1))
    b = np.array([150, 1])
    b = np.tile(b, (num_pts, 1))
    XY = np.hstack((XY, np.vstack((a, b))))
    rhs = np.dot(camera_matrix, XY.T)
    image = np.vstack((front_image, back_image))
    diff = image.T - rhs[:2, :]
    return np.sqrt(np.sum(np.square(diff)) / (2*num_pts))
...
if __name__ == '__main__':
    ...
    print(real_XY.shape, front_image.shape, back_image.shape)
    import pdb
    pdb.set_trace()
    ...
    # (b)
    # ('RMS Error: ', 0.99383048327984413)
    # (c) no, degenerate case
