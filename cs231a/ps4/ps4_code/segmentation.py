...
def kmeans_segmentation(im, features, num_clusters):
    ...
    centers = features[np.random.randint(0, h, num_clusters), np.random.randint(0, w, num_clusters)]
    error = 1e6
    while error > 1e-3:
      distance = np.zeros((h, w, num_clusters))
      for i in range(num_clusters):
        distance[:,:,i] = np.linalg.norm(features - centers[i], axis=-1)
      assignment = np.argmin(distance, axis=-1)
      nc = np.array([np.mean(features[assignment == i], axis = (0,1)) for i in range(num_clusters)])
      error = np.mean(np.linalg.norm(centers - nc, axis=-1))
      centers = nc
    return assignment
...
def meanshift_segmentation(im, features, bandwidth):
    ...
    centers = []
    while unseen:
      pixel = unseen.keys()[np.random.randint(len(unseen))]
      unseen.pop(pixel)
      mean = features[pixel[1], pixel[0]]
      mean_size = 1
      error = 1e6
      while error > 1e-3 and unseen:
        error = 1e-8
        for (x, y) in unseen.keys():
          if np.linalg.norm(mean - features[y, x]) < bandwidth:
            del unseen[(x, y)]
            mean_size += 1
            new_mean = mean * (mean_size-1.0)/mean_size + features[y, x] / mean_size
            error = np.linalg.norm(mean - new_mean)
            mean = new_mean

      # merge clusters
      for i in range(len(centers)-1, -1, -1):
        center, size = centers[i]
        if np.linalg.norm(mean - center) < bandwidth:
          centers.pop(i)
          total = size + mean_size
          mean = mean * mean_size / total + center * size / total
          mean_size = total
      centers.append((mean, mean_size))
    centers = np.array([mean for mean, size in centers])
