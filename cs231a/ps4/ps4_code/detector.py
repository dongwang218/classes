...
def run_detector(im, clf, window_size, cell_size, block_size, nbins, thresh=1):
    ...
    for y in range(0, h-wh, stride):
      for x in range(0, w-ww, stride):
        X = compute_hog_features(im[y:y+wh, x:x+ww], cell_size, block_size, nbins).reshape((1, -1))
        Y = clf.decision_function(X)
        if Y[0] > thresh:
          bboxes.append([x, y, ww, wh])
          scores.append(Y[0])
...
'''
def is_inside(bbox, pt):
  return pt[0] > bbox[0] and pt[0] < bbox[0]+bbox[2] and pt[1] > bbox[1] and pt[1] < bbox[1]+bbox[3]
...
def non_max_suppression(bboxes, confidences):
    ...
    for index in indexes:
      ...
      found = False
      for bb in result:
        if is_inside(bbox, (bb[0]+bb[2]/2.0, bb[1]+bb[3]/2.0)):
          found = True
          break
      if not found:
        result.append(bbox)
